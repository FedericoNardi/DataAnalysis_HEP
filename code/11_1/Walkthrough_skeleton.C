...
//=============================================================
  ...
  for (int i_bin = 1; i_bin<h_masswindow->GetNbinsX(); i_bin++ ){
    ...
    double Ndata_win = h_data->Integral(h_data->FindBin(125-0.5*masswindow_fullwidth), h_data->FindBin(125+0.5*masswindow_fullwidth));
    double Nbgr_win = h_bgr->Integral(h_bgr->FindBin(125-0.5*masswindow_fullwidth), h_bgr->FindBin(125+0.5*masswindow_fullwidth));
    double Nsig_win = h_sig->Integral(h_sig->FindBin(125-0.5*masswindow_fullwidth), h_sig->FindBin(125+0.5*masswindow_fullwidth));
  ...
  printf("Luminosity factor: %5.4f \n", Lumi_scalefactor);

  int MaxBin_exp = h_masswindow_expected->GetMaximumBin();
  double OptimumSign_exp = h_masswindow_expected->GetBinContent(MaxBin_exp);
  double OptMassWindow_exp = h_masswindow_expected->GetXaxis()->GetBinCenter(MaxBin_exp);

  printf("EXPECTED significance - Optimal mass window: \n");
  printf("	Expected significance: %.2f", OptimumSign_exp);
  printf("	Mass window: %.2f GeV \n", OptMassWindow_exp );

  if(fabs(Lumi_scalefactor-1.0)<0.01){
  	int MaxBin_obs = h_masswindow_observed->GetMaximumBin();
  	double OptimumSign_obs = h_masswindow_observed->GetBinContent(MaxBin_exp);
  	double OptMassWindow_obs = h_masswindow_observed->GetXaxis()->GetBinCenter(MaxBin_exp);

  	printf("OBSERVED significance - Optimal mass window: \n");
  	printf("	Expected significance: %.2f", OptimumSign_obs);
  	printf("	Mass window: %.2f GeV \n", OptMassWindow_obs);
...
//===========================
  ...
  for (int i_bin_sf = 1; i_bin_sf <=h_scalefactor_bgr->GetNbinsX(); i_bin_sf++){
    ...
    for (int i_bin = 1; i_bin <=  h_data->GetNbinsX(); i_bin++){
    	...
    	// restrict to signal-free region
    	double m4lepBin = h_data->GetBinCenter(i_bin);
    	int NObsBin = h_data->GetBinContent(i_bin);

    	if(m4lepBin<=150. && m4lepBin<=400.){
    		double MeanBgdBin = scalefactor_bgr * h_bgr->GetBinContent(i_bin);
    		if( MeanBgdBin > 0.){
    			loglik += TMath::Log( TMath::Poisson(NObsBin,MeanBgdBin));
    		}
    	}
    ...
    double Minimum = 1e6;
  	double MinLog;
  	double BestSF_bgd;
  	int NbinsSF = h_scalefactor_bgr->GetNbinsX();
    
    //-- Find minimum
    for ( int i=1; i<=NbinsSF; i++){
    	MinLog = h_scalefactor_bgr->GetBinContent(i);
    	if(MinLog < Minimum){
    		Minimum = MinLog;
    		BestSF_bgd = h_scalefactor_bgr->GetBinCenter(i);
    	}
    }
    ...
    for( int i=1; i<=NbinsSF; i++ ){
    	h_scalefactor_bgr_rescaled->SetBinContent(i,h_scalefactor_bgr->GetBinContent(i)-Minimum);
    	if( LeftLim < 0. && h_scalefactor_bgr_rescaled->GetBinContent(i)<1.00 ){
    		LeftLim = h_scalefactor_bgr->GetBinCenter(i);
    	}
    	if( LeftLim > 0. && RightLim < 0. && h_scalefactor_bgr_rescaled->GetBinContent(i)<1.00 ){
    		RightLim = h_scalefactor_bgr->GetBinCenter(i);
    	}
    }

    //-- print summary to screen
    double LeftError = BestSF_bgd - LeftLim;
    double RightError = RightLim - BestSF_bgd;

    printf("	----------\n");
    printf("	Result fit: \n");
    printf("	----------\n");
    printf("	Background scale factor from sidend fit = %5.2f - %5.2f + %5.2f\n",BestSF_bgd,LeftError,RightError);
